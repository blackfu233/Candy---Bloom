...
function addSpecialEffects(keys) {
  ...
  while (queue.length) {
    const { row, col, triggerType } = queue.shift();
    const tile = board[row][col];
...
async function clearResolvedKeys(initialKeys, initialSpecials = 0, firstLabel = "") {
  ...
  return { totalCleared, totalSpecials, multiplier: payoutMultiplier, cascade };
}
...
function settleBottomFeatures(maxRounds = 4) {
  let totalDrops = 0;
