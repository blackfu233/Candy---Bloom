...
const feverCapsulesEl = document.querySelector("#fever-capsules");
...
let audio = null;
let fallMap = new Map();
let feverRunning = false;
const lowFx = window.matchMedia("(max-width: 520px), (max-height: 760px), (prefers-reduced-motion: reduce)").matches;
...
function wait(ms) {
  const speed = turboEl.checked ? Math.max(40, ms * 0.35) : ms;
  return new Promise((resolve) => setTimeout(resolve, lowFx ? Math.min(speed, 230) : speed));
}
...
function makeCandy(type = randomInt(candyTypes)) {
  return {
    type,
    special: Math.random() < 0.018 ? ["rocket-h", "rocket-v", "bomb"][randomInt(3)] : null,
    feature: null,
    locked: false,
    plastic: false,
    crate: false,
...
function maybeDecorate(tile, row) {
  if (feverRunning) return tile;
  const roll = Math.random();
  if (roll < 0.008 && row < size - 1) tile.feature = "spinner";
  return tile;
}
...
function newBoard() {
  ...
  for (let row = 0; row < size; row += 1) {
    for (let col = 0; col < size; col += 1) {
      ...
      next[row][col] = makeCandy(type);
    }
  }

  addFeatureRain(next, 1);
  return next;
}
...
function render() {
  ...
  const capsuleCount = countFeverCapsules();
  const cappedCapsules = Math.min(capsuleCount, feverTarget);
  const feverTitleEl = document.querySelector(".fever-focus strong");
  if (feverTitleEl) feverTitleEl.textContent = `Capsules ${cappedCapsules}/${feverTarget}`;
  if (feverCapsulesEl) {
    feverCapsulesEl.setAttribute("aria-valuetext", `${cappedCapsules}/${feverTarget} capsules`);
    [...feverCapsulesEl.children].forEach((slot, index) => {
      slot.classList.toggle("filled", index < cappedCapsules);
      slot.classList.toggle("ready", cappedCapsules >= feverTarget);
    });
  }
  ...
  for (const group of groups) {
    ...
    if (group.length >= 5) {
      ...
      creations.set(key, {
        row: point.row,
        col: point.col,
        special: orientation === "h" ? "rocket-v" : "rocket-h",
        type: groupType(group),
      });
    }
  }
...
async function clearResolvedKeys(initialKeys, initialSpecials = 0, firstLabel = "") {
  ...
  while (pendingKeys.size && cascade < maxCascadeSteps) {
    ...
    const feverGain = feverGainFor(clearKeys.size, cascade, specialCount);
    addFever(feverGain, feverReason(clearKeys.size, cascade, specialCount), false);
    await wait(460);
    collapse(clearKeys);
...
function showFeverTick(gain) {
  const tick = document.createElement("span");
  tick.className = "fever-tick";
  tick.textContent = `+${Math.round(gain)}`;
  document.querySelector(".fever-focus")?.append(tick);
  setTimeout(() => tick.remove(), 780);
}

function feverGainFor(cleared, cascade, specials = 0) {
  const base = 1 + Math.max(0, cleared - 3) + Math.max(0, cascade - 1) * 2 + specials * 4;
  return Math.min(15, base);
}

function feverReason(cleared, cascade, specials = 0) {
  const parts = [];
  if (cleared > 3) parts.push(`${cleared} candies`);
  if (cascade > 1) parts.push(`Cascade x${cascade}`);
  if (specials > 0) parts.push(`${specials} special`);
  return parts.length ? parts.join(" + ") : "Match charge";
}
...
function countFeverCapsules() {
  let count = 0;
  for (let row = 0; row < size; row += 1) {
    for (let col = 0; col < size; col += 1) {
      if (board[row][col]?.feature === "spinner") count += 1;
    }
  }
  return count;
}

function feverCapsuleKeys(limit = feverTarget) {
  const keys = [];
  for (let row = 0; row < size; row += 1) {
    for (let col = 0; col < size; col += 1) {
      if (board[row][col]?.feature === "spinner") keys.push(keyOf(row, col));
    }
  }
...
function consumeFeverCapsules() {
  const keys = feverCapsuleKeys(size * size);
  keys.forEach((key) => {
    const { row, col } = parseKey(key);
    board[row][col] = makeCandy(randomInt(candyTypes));
  });
  return keys.length;
}

function addFever(amount, reason = "") {
  if (reason && !feverRunning) messageEl.textContent = `${reason}: capsules stay on board. Keep 3 to spin Fever Wheel.`;
}

function triggerBottomFeatures() {
  return countFeverCapsules();
}

function settleBottomFeatures() {
  return countFeverCapsules();
}
function feverSeedSpecials(count) {
  const seeded = [];
  for (let index = 0; index < count; index += 1) {
    for (let attempt = 0; attempt < 40; attempt += 1) {
      ...
      const tile = board[row][col];
      if (!tile || tile.feature || tile.special) continue;
      tile.special = ["rocket-h", "rocket-v", "bomb"][randomInt(3)];
      seeded.push({ row, col });
      break;
    }
  }
...
function collectBoardSpecialKeys(limit = 12) {
  const keys = [];
  for (let row = 0; row < size; row += 1) {
    for (let col = 0; col < size; col += 1) {
      const tile = board[row][col];
      if (tile?.special && !tile.feature) keys.push(keyOf(row, col));
    }
  }
  keys.sort(() => Math.random() - 0.5);
...
function showFeverFinal(totalAward, totalCleared, totalSpecials, totalCascades) {
  const card = document.createElement("div");
  card.className = "feature-card fever-final-card";
  card.innerHTML = `Candy Fever<small>${totalCleared} candies · ${totalSpecials} specials · Cascade x${totalCascades}<br>Win ${totalAward.toFixed(2)} BET</small>`;
  document.querySelector(".board-wrap").append(card);
  setTimeout(() => card.remove(), 2600);
}

const feverWheelOptions = [
  { name: "3 Free Cascades", short: "FREE", kind: "free-cascades", weight: 0.36, stop: "0deg", index: 0 },
  { name: "Rocket Rain", short: "ROCKET", kind: "rocket-rain", weight: 0.26, stop: "-90deg", index: 1 },
  { name: "Bomb Party", short: "BOMB", kind: "bomb-party", weight: 0.2, stop: "-180deg", index: 2 },
  { name: "Color Crush", short: "COLOR", kind: "color-crush", weight: 0.18, stop: "-270deg", index: 3 },
];

function feverWheelPrize() {
  let roll = Math.random();
  for (const option of feverWheelOptions) {
    roll -= option.weight;
    if (roll <= 0) return option;
  }
  return feverWheelOptions[feverWheelOptions.length - 1];
}

function showFeverWheel(prize) {
  const card = document.createElement("div");
  card.className = "wheel-card fever-bonus-card fever-wheel-card is-spinning";
  card.style.setProperty("--wheel-stop", prize.stop);
  card.innerHTML = `
    <strong>Fever Wheel</strong>
    <div class="fever-wheel-stage">
      <span class="wheel-pointer"></span>
      <div class="fever-prize-wheel">
        ${feverWheelOptions.map((option, index) => `<b class="wheel-option option-${index}" data-index="${index}">${option.short}</b>`).join("")}
      </div>
      <span class="wheel-hub">PICK</span>
    </div>
    <small class="wheel-result-text">Picking...</small>
  `;
  document.querySelector(".board-wrap").append(card);
  playSound("wheel");

  const options = [...card.querySelectorAll(".wheel-option")];
  options[0]?.classList.add("active");
  const hub = card.querySelector(".wheel-hub");
  const resultText = card.querySelector(".wheel-result-text");
  const sequence = [0, 1, 2, 3, 1, 0, 2, 3, 0, 1, 3, 2, 0, 3, 1, 2, prize.index];
  const delays = [55, 55, 60, 60, 65, 70, 75, 80, 90, 100, 115, 135, 155, 185, 230, 280, 0];
  let elapsed = 0;
  sequence.forEach((index, step) => {
    elapsed += delays[step];
    setTimeout(() => {
      options.forEach((option) => option.classList.remove("active", "selected"));
      options[index]?.classList.add(step === sequence.length - 1 ? "selected" : "active");
      if (hub) hub.textContent = step === sequence.length - 1 ? "WIN" : "PICK";
      if (resultText && step === sequence.length - 1) resultText.textContent = prize.name;
      if (step === sequence.length - 1) card.classList.remove("is-spinning");
    }, elapsed);
  });
...
function feverRevealDetail(prize) {
  if (prize.kind === "free-cascades") return "3 free waves are charging the board";
  if (prize.kind === "rocket-rain") return "Rockets are locking onto the board";
  if (prize.kind === "bomb-party") return "Bombs are arming across the board";
  return "One candy color is being crushed";
}

function showFeverPrizeReveal(prize) {
  const wrap = document.querySelector(".board-wrap");
  const card = document.createElement("div");
  card.className = `fever-prize-reveal reveal-${prize.kind}`;
  card.innerHTML = `
    <span>FEVER PRIZE</span>
    <strong>${prize.name}</strong>
    <small>${feverRevealDetail(prize)}</small>
  `;
  wrap.append(card);

  const sweep = document.createElement("div");
  sweep.className = `fever-prize-sweep sweep-${prize.kind}`;
  wrap.append(sweep);

  wrap.classList.add("fever-prize-armed");
  playSound("fever");
  setTimeout(() => pulseBoard(), 250);
  setTimeout(() => pulseBoard(), 760);
  setTimeout(() => {
    card.remove();
    sweep.remove();
    wrap.classList.remove("fever-prize-armed");
  }, 1600);
}
function collectColorKeys(targetType) {
  const keys = new Set();
  for (let row = 0; row < size; row += 1) {
    for (let col = 0; col < size; col += 1) {
      const tile = board[row][col];
      if (tile && !tile.feature && tile.type === targetType) keys.add(keyOf(row, col));
    }
  }
  return keys;
}

async function runFeverFreeCascades() {
  let totalCleared = 0;
  let totalSpecials = 0;
  let totalCascades = 0;
  let totalAward = 0;
  ...
  for (let wave = 0; wave < 3; wave += 1) {
    const seeded = feverSeedSpecials(4 + wave);
    cascadeLabelEl.textContent = `Free Cascade ${wave + 1}/3`;
    messageEl.textContent = `Free Cascade ${wave + 1}: ${seeded.length} free specials dropped in.`;
    render();
    markTransform(new Set(seeded.map(({ row, col }) => keyOf(row, col))));
    pulseBoard();
    playSound("create", wave + 1);
    await wait(720);

    const starterKeys = collectBoardSpecialKeys(8 + wave * 2);
    const result = await clearResolvedKeys(starterKeys, 0, `Free Cascade ${wave + 1}`);
    const feverExtra = result.win * 2;
    totalCleared += result.totalCleared;
    totalSpecials += result.totalSpecials;
    totalCascades += result.cascade;
    totalAward += result.win + feverExtra;
    lastWin += feverExtra;
    balance += feverExtra;
    messageEl.textContent = `Free Cascade ${wave + 1}: base ${result.win.toFixed(2)} + Fever x3 bonus ${feverExtra.toFixed(2)}.`;
    render();
    await wait(820);
  }

  return { totalCleared, totalSpecials, totalCascades, totalAward };
}

async function runFeverBoardEffect(prize) {
  if (prize.kind === "free-cascades") return runFeverFreeCascades();

  if (prize.kind === "rocket-rain") feverSeedSpecials(9).forEach(({ row, col }) => board[row][col].special = randomInt(2) ? "rocket-h" : "rocket-v");
  if (prize.kind === "bomb-party") feverSeedSpecials(7).forEach(({ row, col }) => board[row][col].special = "bomb");

  let starterKeys = collectBoardSpecialKeys(prize.kind === "bomb-party" ? 10 : 12);
  if (prize.kind === "color-crush") {
    starterKeys = collectColorKeys(randomInt(candyTypes));
  }

  render();
  markPreview(starterKeys);
  pulseBoard();
  playSound("special");
  await wait(760);
  const result = await clearResolvedKeys(starterKeys, 0, prize.name);
  const feverExtra = result.win * 2;
  lastWin += feverExtra;
  balance += feverExtra;
  return {
...
async function triggerFeverMode() {
  if (countFeverCapsules() < feverTarget || feverRunning) return;
  feverRunning = true;
  locked = true;
  const wrap = document.querySelector(".board-wrap");
  consumeFeverCapsules();
  const prize = feverWheelPrize();

  cascadeLabelEl.textContent = "Fever Wheel";
  messageEl.textContent = "3 Fever Capsules collected. Spinning Fever Wheel.";
  wrap?.classList.add("fever-surge");
  showFeverWheel(prize);
  pulseBoard();
  render();
  await wait(3300);

  cascadeLabelEl.textContent = prize.name;
  messageEl.textContent = `Fever Wheel landed on ${prize.name}.`;
  showFeverPrizeReveal(prize);
  await wait(1450);
  const outcome = await runFeverBoardEffect(prize);
  showFeverFinal(outcome.totalAward, outcome.totalCleared, outcome.totalSpecials, outcome.totalCascades);
  showSettlement({
    totalCleared: outcome.totalCleared,
    totalSpecials: outcome.totalSpecials,
    multiplier: 3,
    specialBase: 3,
    cascadeBonus: Math.max(0, outcome.totalCascades - 1) * 0.08,
    baseWin: outcome.totalCleared * candyValue(),
    win: outcome.totalAward,
    cascade: outcome.totalCascades,
  });
  consumeFeverCapsules();
  const finalMessage = `${prize.name}: Fever paid x3 for ${outcome.totalAward.toFixed(2)} BET.`;
  messageEl.textContent = finalMessage;
  render();
  await wait(1900);
  wrap?.classList.remove("fever-surge");
...
async function clearCascade() {
  ...
  while (pendingGroups.length && cascade < maxCascadeSteps) {
    ...
    const feverGain = feverGainFor(clearKeys.size, cascade, specialCount + creations.length);
    addFever(feverGain, feverReason(clearKeys.size, cascade, specialCount + creations.length), false);
    await wait(460);
    collapse(clearKeys);
    triggerBottomFeatures();
    render();
...
function shuffleGeneralCandies(cost = 0) {
  ...
  selected = null;
  hintPair = null;
  messageEl.textContent = cost ? "Shuffle used. Fever Capsules stayed in place." : "No legal move: free reshuffle.";
...
async function handleCellClick(event) {
  ...
  if (swapAction) {
    ...
    settleBottomFeatures();
    if (countFeverCapsules() >= feverTarget) {
      await triggerFeverMode();
...
function resetGame() {
  ...
  hintPair = null;
  cascadeLabelEl.textContent = "";
  messageEl.textContent = "Keep 3 Fever Capsules on the board to spin the Fever Wheel.";
  settleBottomFeatures();
  render();
}
...
hintEl.addEventListener("click", () => {
  ...
  messageEl.textContent = hintPair ? "Hint highlighted. Build Fever with a legal swap." : "No legal swap found. Reshuffling.";
  if (!hintPair) shuffleGeneralCandies(0);
  render();
});


function demoFeverFromQuery() {
  const params = new URLSearchParams(window.location.search);
  if (!params.has("demoFever")) return;
  setTimeout(() => {
    while (countFeverCapsules() < feverTarget) addFeatureRain(board, 1);
    render();
    triggerFeverMode();
  }, 700);
}
